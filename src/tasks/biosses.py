...
class CustomTask(BaseTask):
    ...
    def clean_response(self, response):
        boxed_pattern_abc = r"\\boxed{(A|B|C)}"
        match = re.findall(boxed_pattern_abc, response)
        if len(match) != 0:
            return match[-1]

        boxed_pattern_text = r"\\boxed{(not similar|somewhat similar|similar)}"
        match = re.findall(boxed_pattern_text, response.lower())
        if len(match) != 0:
            return self.option_dict[match[-1]]

        clean_pattern_abc = r"\((A|B|C)\)"
        match = re.findall(clean_pattern_abc, response)
        if len(match) != 0:
            return match[-1]

        clean_pattern_text = r"\b(not similar|somewhat similar|similar)\b"
        match = re.findall(clean_pattern_text, response.lower())
        if len(match) != 0:
            return self.option_dict[match[-1]]
